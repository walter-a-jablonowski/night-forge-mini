"""Analyze step: context -> (finding, measured metric, proposed actions).

A single swappable function (idea_2 seam). Real mode asks the model for a JSON
proposal; `--fake-llm` mode is deterministic (one add_entry per new snippet) so the
...
from .actions import ACTIONS, slug
...
You may ONLY propose these actions: {actions}.
  add_entry(target=new-entry-id, payload={{title, body, source}})         -- new knowledge
...
def analyze(model: ModelWrapper, *, goal: str, kb_index: list[dict],
            snippets: list[dict], recent_findings: list[str]) -> dict[str, Any]:
    if model.fake:
        actions = _fake_actions(kb_index, snippets)
        finding = f"{len(snippets)} new snippet(s); {len(actions)} proposed add(s) [fake-llm]"
        return {"finding": finding, "actions": actions, "model": "fake-llm"}
...
def _fake_actions(kb_index: list[dict], snippets: list[dict]) -> list[dict]:
    ...
    for s in snippets:
        ...
        if target in existing:
            # snippet updates a topic already in the KB -> overwrite curated content.
            # edit_entry is reversible=False, so the gate holds it for approval.
            actions.append({
