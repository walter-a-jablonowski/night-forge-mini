...
## Anti-spin still applies
`Store.last_run_made_progress()` guards the `pending_work` trigger and must guard this one
too, or a metric that is stuck below target re-runs forever. A metric that does not move
after a pass is exactly the "no progress" case.
